...
(function($, window, document, undefined) {
	...
	ThemifyPageBuilder = {
		...
		bindEvents: function() {
			...
			.on('dblclick', '#themify_builder_overlay', function(){
				...
				resize: function( event, ui ){
					...
					if(self.getDirection(ui) == 'right'){
						if( gridString == '[4-1][4-2][4-1]' && ui.originalSize.width < self.col3_w ) {
							s_temp = ui.originalSize.width + (self.col3_w - ui.originalSize.width + 30);
					...
					if(self.getDirection(ui) == 'left'){
						if( gridString == '[4-2][4-1][4-1]' && ui.originalSize.width > self.col3_w ) {
							s_temp = ui.originalSize.width - (ui.originalSize.width - self.col3_w - 30);
		...
		freezePage : function(){
			ThemifyPageBuilder.freeze_page_scroll = document.body.scrollTop - $('#wpadminbar').outerHeight();
			jQuery( '#wp-link-wrap, #wp-link-backdrop' ).appendTo( 'body' );
			$('body').contents().not('#themify_builder_overlay, #themify_builder_lightbox_parent, #wp-link-wrap, #wp-link-backdrop, #themify_lightbox_fa, #themify_lightbox_overlay').wrapAll( '<div id="builder-fixed-scroll" />' );
			$('#builder-fixed-scroll').css( 'top', '-' + ThemifyPageBuilder.freeze_page_scroll + 'px' );
		},

		unfreezePage : function(){
			$('#builder-fixed-scroll').contents().unwrap();
			document.body.scrollTop = ThemifyPageBuilder.freeze_page_scroll + $('#wpadminbar').outerHeight();
		},
		...
		optionsModule: function( event ) {
			...
			var self = ThemifyPageBuilder,
				...
				is_settings_exist = (self._objectAssocLength(set_elems) > 0) ? true : false,
				el_settings = (is_settings_exist) ? set_elems : '',
				options = {
					data: {
